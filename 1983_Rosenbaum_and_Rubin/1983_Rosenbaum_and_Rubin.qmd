...
$$Pr(Z=1|b(x)) = e(x).$$
Why is this the case?  Because $e(x)$ is a balancing score, as evidenced by
the fact that $Z$ is binary and that $e(x) = Pr(Z=1|X=x)$ completely
characterizes the interdependence of $X$ and $Z$.  Given that, conditioning
on $e(x)$ is analagous to conditioning on a particular value $X=x$. $X
\independent Z \rvert X = x$ is of course true given that $x$ is a fixed
value independent of $Z$. Similarly we have that $X \independent Z \rvert
Pr(Z = 1 | X = x)$.
...
what bias results from dividing the dataset into quintiles. 
